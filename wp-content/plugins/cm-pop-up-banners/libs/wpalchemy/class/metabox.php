...
class WPAlchemy_MetaBox {
     ...
     * @access	public
     */
    function get_the_value($n = NULL, $collection = FALSE, $mode = NULL) {
        ...
        if (is_string($value) || is_numeric($value)) {
            if ($this->in_template) {
				if(is_null($mode)) {
					return htmlentities($this->cmpopupbanners_striptags($value), ENT_QUOTES, 'UTF-8');
				} else {
                ...
                global $wp_embed;
				if(is_null($mode)) {
					return do_shortcode($wp_embed->run_shortcode($this->cmpopupbanners_striptags($value)));
				} else {
			...
			if(is_null($mode)) {
            return $this->cmpopupbanners_striptags($value);
			} else {
				return $value;
			}
        }
	...
	function cmpopupbanners_striptags($val = '') {
		$val = strip_tags($val);
