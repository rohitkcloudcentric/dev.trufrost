	...
	<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
		<thead>
			<tr>
				...
				<th class="product-name"><?php esc_html_e( 'Product', 'wcpb' ); ?></th>
				<th class="product-price"><?php esc_html_e( 'Price', 'wcpb' ); ?></th>
				<th class="product-quantity"><?php esc_html_e( 'Quantity', 'wcpb' ); ?></th>
				<th class="product-subtotal"><?php esc_html_e( 'Total', 'wcpb' ); ?></th>
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); echo ' '.$product_bundle_class ?>">
								...
								?>
									...
									<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'wcpb' ); ?>">
										<?php
											if ( ! $product_permalink ) {
											...
											if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
												echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'wcpb' ) . '</p>' ) );

											}
										?>
									</td>
								<?php else:?>
									<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'wcpb' ); ?>" colspan="2">
										<?php
											if ( ! $product_permalink ) {
											...
											// Backorder notification
											if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
												echo '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'wcpb' ) . '</p>';
											}
										?>
									</td>
								<?php endif;?>

								<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'wcpb' ); ?>">
									<?php
								...
								<?php
									if ( ! isset( $cart_item['bundle-parent'] ) ) :
								?>
									<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'wcpb' ); ?>">
										<?php
								...
								<?php endif;?>
								<td class="product-subtotal" data-title="<?php esc_attr_e( 'Total', 'wcpb' ); ?>">
									<?php
									...
									<?php
										if ( version_compare( WC_VERSION, '3.3.0', '<' ) ) {
											echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
												...
												esc_html__( 'Remove this item', 'wcpb' ),
												esc_attr( $product_id ),
				...
				<td colspan="6" class="actions">
					...
					<?php if ( wc_coupons_enabled() ) { ?>
						<div class="coupon flex middle-xs">
							...
							<label for="coupon_code"><?php _e( 'Coupon:', 'wcpb' ); ?></label> <input type="text" name="coupon_code" class="input-text br__40" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'wcpb' ); ?>" /> <button type="submit" class="button" name="apply_coupon" value="<?php esc_attr_e( 'Apply Coupon', 'wcpb' ); ?>"><?php esc_attr_e( 'Apply coupon', 'wcpb' ); ?></button>

							<?php do_action( 'woocommerce_cart_coupon' ); ?>
						</div>
					<?php } ?>

					<button type="submit" class="button update-cart" name="update_cart" value="<?php esc_attr_e( 'Update Cart', 'wcpb' ); ?>"><?php esc_attr_e( 'Update Cart', 'wcpb' ); ?></button>
