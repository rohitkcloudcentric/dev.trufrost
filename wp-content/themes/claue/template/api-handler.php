...
function sendWhatsAppOTP($mobileNumber, $otpCode)
{
    ...
    $apiVersion = defined('META_API_VERSION') ? META_API_VERSION : 'v18.0';
    ...
    $wabaId = defined('META_WABA_ID') ? META_WABA_ID : '';
    ...
    // Determine template name dynamically based on WABA ID or custom constant
    $templateName = 'otp';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'submitServiceRequest':
            ...
            // Store complete data in database before sending to Salesforce
            try {
                $table_name = $wpdb->prefix . 'service_requests';

                // For MockWPDB / Standalone testing
                if ($wpdb instanceof MockWPDB) {
                    if (!isset($_SESSION['mock_service_requests'])) {
                        $_SESSION['mock_service_requests'] = [];
                    }
                    $_SESSION['mock_service_requests'][] = [
                    ...
                    $inserted_id = count($_SESSION['mock_service_requests']);
                    error_log('Mock DB: Service Request stored successfully. ID: ' . $inserted_id);
                } else {
                    ...
                    if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
                        ...
                        $sql = "CREATE TABLE $table_name (
                            ...
                            mobile_number varchar(20) DEFAULT '' NOT NULL,
                            customer_name varchar(255) DEFAULT '' NOT NULL,
                            email varchar(255) DEFAULT '' NOT NULL,
                            form_data longtext DEFAULT '' NOT NULL,
                            salesforce_status varchar(50) DEFAULT 'pending' NOT NULL,
                            salesforce_response text DEFAULT NULL,
                    ...
                    if ($inserted) {
                        ...
                        error_log('WordPress DB: Service Request stored successfully. ID: ' . $inserted_id);
                    } else {
                        error_log('WordPress DB Error storing Service Request: ' . $wpdb->last_error);
                    }
                }
            ...
            try {
                if (isset($inserted_id)) {
                    $status = ($result && (
                        ...
                        (isset($result['status']) && strcasecmp($result['status'], 'success') === 0) ||
                        isset($result['parentSRId']) ||
                        isset($result['caseId'])
                    )) ? 'success' : 'failed';
                    $response_json = json_encode($result);

                    if ($wpdb instanceof MockWPDB) {
                        if (isset($_SESSION['mock_service_requests'][$inserted_id - 1])) {
                            $_SESSION['mock_service_requests'][$inserted_id - 1]['salesforce_status'] = $status;
                            $_SESSION['mock_service_requests'][$inserted_id - 1]['salesforce_response'] = $response_json;
                        }
                    } else {
                        $wpdb->update(
                            $table_name,
                ...
                error_log('Error updating service request status in DB: ' . $dbEx->getMessage());
            }
